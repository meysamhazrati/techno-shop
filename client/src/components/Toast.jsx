...
const Toast = ({ status, title, description, onClose }) => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      toastTimer.current.style.width = `${timerWidth}%`;
    }, 4000 / 100);
    ...
    <div ref={toast} className="fixed left-4 right-4 top-0 z-[60] max-w-full overflow-hidden rounded-2xl bg-white p-6 opacity-0 shadow transition-all duration-300 xs:right-0 xs:top-4 xs:max-w-max lg:left-8 lg:top-8">
      <div className="flex size-full items-center gap-x-4">
        ...
        <div>
          ...
          <p className="mt-0.5 text-lg text-zinc-400">{description || (status === "success" ? "عملیات مورد نظر با موفقیت انجام شد." : "مشکلی پیش آمد! لطفا بعدا دوباره تلاش کنید.")}</p>
        </div>
      </div>
