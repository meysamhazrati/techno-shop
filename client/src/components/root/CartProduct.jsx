...
const CartProduct = ({ product, color }) => {
  return (
    <div className={`grid grid-cols-[150px_1fr] items-center gap-x-4 gap-y-2 overflow-auto py-4 first:pt-0 last:pb-0 ${color.inventory > 0 ? "opacity-100" : "opacity-50"}`}>
      ...
      {color.inventory > 0 && Date.parse(product.offer?.expiresAt) > Date.now() && <AmazingOfferTimer width="full" fontSize="sm" expiresAt={product.offer.expiresAt} />}
      {color.inventory > 0 ? <ProductButton id={product._id} color={color} disabled={!color.inventory} /> : <button></button>}
      <ProductPrice price={color.price} offer={product.offer} priceFontSize="lg" discountedPriceFontSize="sm" gapX="[2px]" iconSize="5" hasInventory={color.inventory} />
    </div>
  );
};
