...
const CategoriesFilter = ({ setFilter }) => {
  ...
  return (
    <div className="pt-3">
      ...
      <div className={`mt-3 ${isOpen ? "max-h-28 border-t border-zinc-200 py-2" : "max-h-0 border-none py-0"} flex flex-col gap-y-1 overflow-auto px-2 transition-all`}>
        {isCategoriesError ? (
          ...
          <div key={_id} className="flex items-center gap-x-3">
            <input
              ...
              className="size-4 shrink-0 cursor-pointer appearance-none rounded bg-zinc-200 checked:bg-primary-900"
              onChange={({ target }) => setFilter((categories) => target.checked ? [...categories, englishTitle.toLowerCase().split(" ").join("-")] : categories.filter((category) => category !== englishTitle.toLowerCase().split(" ").join("-")))}
            />
            <div className="flex w-full items-center justify-between gap-x-3">
