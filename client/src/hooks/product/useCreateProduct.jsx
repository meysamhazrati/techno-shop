...
import { create } from "../../axios/controllers/product";
...
const useCreateProduct = (type) => {
  const client = useQueryClient();

  const { openToast } = useContext(ToastContext);

  const { isPending, mutate } = useMutation({
    mutationFn: (body) => create(type, body),
    onMutate: async (body) => {
      if (type) {
  ...
  return { isPendingCreateProduct: isPending, createProduct: mutate };
};
