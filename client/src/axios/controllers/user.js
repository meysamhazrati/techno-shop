...
const edit = async ({ firstName, lastName, currentPassword, newPassword, avatar }) => await axios.putForm("/users", {
  firstName,
  lastName,
  currentPassword,
  newPassword,
...
const update = async (id, { firstName, lastName, phone, email, password, role, avatar }) => await axios.putForm(`/users/${id}`, {
  firstName,
  lastName,
  phone,
  email,
...
const unBan = async (id) => await axios.put(`/users/unBan/${id}`);
