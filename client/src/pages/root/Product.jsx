...
const Product = () => {
  ...
  const { isFetchingProduct, isProductError, product, hasProductNextPage, fetchProductNextPage } = useProduct(id, true, 10);

  useEffect(() => {
    document.title = isProductError || isFetchingProduct ? "تکنوشاپ" : `تکنوشاپ - محصولات - ${product.title}`;
              ...
              >
                {product.covers.map((cover, index) => (
                  <SwiperSlide key={index}>
                     <img src={`${process.env.SERVER_URI}/images/products/${cover}`} alt={product.title} className="size-full object-cover" />
                  </SwiperSlide>
                ))}
              </Swiper>
            </div>
              ...
              <div className="flex flex-col gap-y-6">
                <div className="flex items-center gap-x-4">
                  <Link to={`/brands/${product.brand.englishName.toLowerCase().split(" ").join("-")}`} className="size-24 shrink-0 overflow-hidden rounded-full">
                    <img src={`${process.env.SERVER_URI}/images/brands/${product.brand.logo}`} alt={product.brand.name} className="size-full object-cover" />
                  </Link>
                  <div>
              ...
              <div className="mt-6">
                {product.colors[0].inventory !== 0 && Date.parse(product.offer?.expiresAt) > Date.now() && (
                  <div className="mb-1.5 flex shrink items-center justify-between gap-x-5">
                    <AmazingOfferTimer expiresAt={product.offer.expiresAt} width="192px" fontSize="18px" />
                  </div>
                )}
                <div className="flex items-center justify-between gap-x-5">
                  <ProductPrice price={color.price || 0} offer={product.offer} hasInventory={color.inventory} priceFontSize="20px" discountedPriceFontSize="16px" columnGap="2px" iconSize="20px" />
