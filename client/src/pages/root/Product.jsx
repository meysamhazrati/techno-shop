              ...
              >
                {product?.covers.map((cover, index) => (
                  <SwiperSlide key={index}>
                     <img src={`http://localhost:3000/products/${cover}`} alt="Product Cover" className="size-full object-cover" />
                  </SwiperSlide>
                ))}
              </Swiper>
            </div>
              ...
              <div className="flex flex-col gap-y-6">
                <div className="flex items-center gap-x-4">
                  <Link to={`/brands/${product?.brand.englishName.toLowerCase().split(" ").join("-")}`} className="size-24 shrink-0 overflow-hidden rounded-full">
                    <img src={`http://localhost:3000/brands/${product?.brand.logo}`} alt="Product Brand" className="size-full object-cover" />
                  </Link>
                  <div>
