  ...
  return !isTicketError && (
    <>
      <div className="mb-6 flex flex-col gap-3 overflow-auto text-nowrap border-b border-zinc-200 pb-6 text-lg sm:flex-row sm:items-center">
        ...
        <div className="hidden size-1.5 shrink-0 rounded-full bg-zinc-400 sm:block"></div>
        <div className="flex items-center gap-x-2">
          <span className="text-zinc-400">فرستنده:</span>
          <span>{isFetchingTicket ? "در حال بارگذاری" : `${ticket.sender.firstName} ${ticket.sender.lastName}`}</span>
        </div>
        <div className="hidden size-1.5 shrink-0 rounded-full bg-zinc-400 sm:block"></div>
        <div className="flex items-center gap-x-2">
      ...
      <div className="mr-auto w-11/12 rounded-br-3xl rounded-tl-3xl rounded-tr-3xl bg-zinc-200 p-4 text-zinc-700">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <span className="text-zinc-500">{isFetchingTicket ? "در حال بارگذاری" : new Intl.DateTimeFormat("fa", { dateStyle: "medium" }).format(Date.parse(ticket.createdAt))}</span>
        </div>
        <p className="my-2 text-lg" dangerouslySetInnerHTML={{ __html: isFetchingTicket ? "در حال بارگذاری" : ticket.body.replaceAll("\n", "<br />") }}></p>
        <span className="text-zinc-500">{isFetchingTicket ? "در حال بارگذاری" : new Intl.DateTimeFormat("fa", { hour: "2-digit", minute: "2-digit", hour12: true }).format(Date.parse(ticket.createdAt))}</span>
      </div>
      {ticket?.replies.map(({ _id, body, sender, createdAt }) => (
        <div key={_id} className={`mt-6 w-11/12 rounded-tl-3xl rounded-tr-3xl p-4 ${sender._id === me._id ? "ml-auto rounded-bl-3xl bg-primary-900 text-white" : "mr-auto rounded-br-3xl bg-zinc-200 text-zinc-700"}`}>
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h6 className="font-vazirmatn-medium text-lg">{sender.firstName} {sender.lastName}</h6>
            <span className={sender._id === me._id ? "text-zinc-200" : "text-zinc-500"}>{new Intl.DateTimeFormat("fa", { dateStyle: "medium" }).format(Date.parse(createdAt))}</span>
          </div>
          <p className="my-2 text-lg" dangerouslySetInnerHTML={{ __html: body.replaceAll("\n", "<br />") }}></p>
          <span className={sender._id === me._id ? "text-zinc-200" : "text-zinc-500"}>{new Intl.DateTimeFormat("fa", { hour: "2-digit", minute: "2-digit", hour12: true }).format(Date.parse(createdAt))}</span>
        </div>
      ))}
      {ticket?.isOpen && (
