...
const Product = () => {
  ...
  const [newHasFingerprintSensor, setNewHasFingerprintSensor] = useState(false);
  const [newHasNoiseCancelling, setNewHasNoiseCancelling] = useState(false);
  const [newIsWaterproof, setNewIsWaterproof] = useState(false);

  const { isFetchingProduct, isProductError, product, hasProductNextPage, fetchProductNextPage } = useProduct(id, 20);
  const { isPendingUpdateProduct, updateProduct } = useUpdateProduct(id, product?.__t);
  const { brands } = useBrands();
  const { categories } = useCategories();

  useEffect(() => {
    document.title = isFetchingProduct || isProductError ? "تکنوشاپ - مدیریت" : `تکنوشاپ - مدیریت - ${product.title}`;

    if (!isFetchingProduct) {
      setNewTitle(product.title);
      setNewWarranty(product.warranty.toString());
      setNewColors(product.colors);
      setNewBrand(product.brand._id);
      setNewCategory(product.category._id);
      setNewLength(product.length?.toString());
      setNewWidth(product.width?.toString());
      setNewHeight(product.height?.toString());
      setNewThickness(product.thickness?.toString());
      ...
      setNewCPUFrequency(product.CPUFrequency?.toString());
      setNewCPUCore(product.CPUCore?.toString());
      setNewGPU(product.GPU);
      setNewGPUModel(product.GPUModel);
      setNewGPURAM(product.GPURAM?.toString());
      setNewOperatingSystem(product.operatingSystem);
      setNewOperatingSystemVersion(product.operatingSystemVersion?.toString());
      setNewInternalMemory(product.internalMemory?.toString());
      setNewInternalMemoryType(product.internalMemoryType);
      setNewDrive(product.drive);
      setNewStorage(product.storage?.toString());
      setNewRAM(product.RAM?.toString());
      setNewRAMType(product.RAMType);
      setNewMemoryCard(product.memoryCard);
      setNewSIMCard(product.SIMCard?.toString());
      setNewNetwork(product.network);
      setNewBattery(product.battery);
      setNewBatteryCapacity(product.batteryCapacity?.toString());
      setNewChargingPort(product.chargingPort);
      setNewChargingSpeed(product.chargingSpeed?.toString());
      setNewScreen(product.screen);
      setNewScreenSize(product.screenSize?.toString());
      setNewResolutionWidth(product.resolutionWidth?.toString());
      setNewResolutionHeight(product.resolutionHeight?.toString());
      setNewCamera(product.camera?.toString());
      setNewPhotoResolution(product.photoResolution?.toString());
      setNewVideoResolutionWidth(product.videoResolutionWidth?.toString());
      setNewVideoResolutionHeight(product.videoResolutionHeight?.toString());
      setNewVideoFPS(product.videoFPS?.toString());
      setNewHeadphoneJack(product.headphoneJack);
      setNewConnectionType(product.connectionType);
      setNewInterfaces(product.interfaces);
      setNewButtons(product.buttons?.toString());
      setNewKeys(product.keys?.toString());
      setNewMinimumDPI(product.minimumDPI?.toString());
      setNewMaximumDPI(product.maximumDPI?.toString());
      setNewMinimumResponseTime(product.minimumResponseTime?.toString());
      setNewMaximumResponseTime(product.maximumResponseTime?.toString());
      setNewPanel(product.panel);
      setNewBacklight(product.backlight);
      setNewColor(product.color?.toString());
      setNewImpedance(product.impedance?.toString());
    ...
    !isProductError && (
      <>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {isFetchingProduct ? Array(4).fill().map((cover, index) => <div key={index} className="h-56 w-full animate-pulse rounded-3xl bg-skeleton"></div>) : product.covers.map((cover, index) => <img key={index} src={`${process.env.SERVER_URI}/images/products/${cover}`} alt="Product Cover" loading="lazy" className="h-56 w-full rounded-3xl object-cover" />)}
        </div>
        <div className="mt-4 flex flex-col gap-3 overflow-auto text-nowrap text-lg sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-x-2">
            ...
            <span>{isFetchingProduct ? "در حال بارگذاری" : `${product.warranty} ماه`}</span>
          </div>
          <div className="flex items-center gap-x-2">
        ...
        <div className="mt-3 flex flex-col gap-3 text-nowrap text-lg">
          ...
          )) : product.colors.map(({ _id, price, inventory, name, code }) => (
        ...
        <form className="mt-4 text-lg" onSubmit={(event) => {
          ...
          updateProduct(Object.fromEntries(Object.entries({
            ...
            chip: newChip?.trim(),
            motherboard: newMotherboard?.trim(),
            CPU: newCPU?.trim(),
            CPUSeries: newCPUSeries?.trim(),
            CPUGeneration: newCPUGeneration?.trim(),
            CPUFrequency: newCPUFrequency?.trim(),
            CPUCore: newCPUCore?.trim(),
            GPU: newGPU?.trim(),
            GPUModel: newGPUModel?.trim(),
            GPURAM: newGPURAM?.trim(),
            operatingSystem: newOperatingSystem?.trim(),
            operatingSystemVersion: newOperatingSystemVersion?.trim(),
            internalMemory: newInternalMemory?.trim(),
            internalMemoryType: newInternalMemoryType?.trim(),
            drive: newDrive?.trim(),
            storage: newStorage?.trim(),
            RAM: newRAM?.trim(),
            RAMType: newRAMType?.trim(),
            memoryCard: newMemoryCard?.trim(),
            SIMCard: newSIMCard?.trim(),
            network: newNetwork?.trim(),
            battery: newBattery?.trim(),
            batteryCapacity: newBatteryCapacity?.trim(),
            chargingPort: newChargingPort?.trim(),
            chargingSpeed: newChargingSpeed?.trim(),
            screen: newScreen?.trim(),
            screenSize: newScreenSize?.trim(),
            resolutionWidth: newResolutionWidth?.trim(),
            resolutionHeight: newResolutionHeight?.trim(),
            camera: newCamera?.trim(),
            photoResolution: newPhotoResolution?.trim(),
            videoResolutionWidth: newVideoResolutionWidth?.trim(),
            videoResolutionHeight: newVideoResolutionHeight?.trim(),
            videoFPS: newVideoFPS?.trim(),
            headphoneJack: newHeadphoneJack?.trim(),
            connectionType: newConnectionType?.trim(),
            interfaces: newInterfaces,
            buttons: newButtons?.trim(),
            keys: newKeys?.trim(),
            minimumDPI: newMinimumDPI?.trim(),
            maximumDPI: newMaximumDPI?.trim(),
            minimumResponseTime: newMinimumResponseTime?.trim(),
            maximumResponseTime: newMaximumResponseTime?.trim(),
            panel: newPanel?.trim(),
            backlight: newBacklight?.trim(),
            color: newColor?.trim(),
            impedance: newImpedance?.trim(),
            strapMaterial: newStrapMaterial?.trim(),
          ...
          <div className="mt-4 overflow-auto rounded-3xl border border-zinc-200">
            <table className="w-full text-nowrap text-lg">
              ...
              <InfiniteScroll hasNextPage={hasProductNextPage} fetchNextPage={fetchProductNextPage}>
                <tbody>
                  {product?.comments.map((comment) => <Comment key={comment._id} {...comment} product={product} />)}
