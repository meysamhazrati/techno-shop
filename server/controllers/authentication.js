...
const register = async (request, response, next) => {
  try {
    ...
    const isUserExists = await userModel.findOne({ phone }).lean();

    if (isUserExists) {
      response.status(409).json({ message: "This phone already exists." });
    } else {
      ...
      const token = jwt.sign({ id }, process.env.JWT_SECRET, { expiresIn: "30 day" });
...
const login = async (request, response, next) => {
  try {
    ...
    if (user) {
      ...
      const isUserBanned = await bannedUserModel.findOne({ $or: [{ phone: identifier }, { email: identifier }] }).lean();
      ...
      if (isUserBanned) {
        ...
        if (isPasswordValid) {
          ...
          response.status(403).json({ message: "The password is incorrect." });
        }
      }
    } else {
